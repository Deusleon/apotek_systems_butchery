...
class OutGoingStockController extends Controller {
    ...
    public function showOutStock(Request $request)
    {
        if ($request->ajax()) {
            ...
            $stockTracking = StockTracking::with('product')
                ->whereBetween('updated_at', [
                    date('Y-m-d', strtotime($request->date_from)),
                    date('Y-m-d', strtotime($request->date)),
                ])
                ->when(!is_all_store(), fn($q) => $q->where('store_id', $default_store))
                ->get();

            // ✅ Group by stock_id
            $groupedByStock = $stockTracking->groupBy('stock_id')->map(function ($rows) use ($default_store) {
                $stockId   = $rows->first()->stock_id;
                $productId = $rows->first()->product_id;

                // current stock by stock
                $currentStockByStockQuery = DB::table('inv_current_stock')->where('id', $stockId);
                if (!is_all_store()) {
                    $currentStockByStockQuery->where('store_id', $default_store);
                }
                $currentStockByStock = $currentStockByStockQuery->sum('quantity');

                return [
                    'stock_id'      => $stockId,
                    'product_id'    => $productId,
                    'product_name'  => $rows->first()->product->name ?? null,
                    'in_total'      => $rows->where('movement', 'IN')->sum('quantity'),
                    'out_total'     => $rows->where('movement', 'OUT')->sum('quantity'),
                    'current_stock' => $currentStockByStock,
                    'in_movements'  => $rows->where('movement', 'IN')->map(function ($item) {
                        return [
                            'id'   => $item->id,
                            'qty'  => $item->quantity,
                    ...
                    'out_movements' => $rows->where('movement', 'OUT')->map(function ($item) {
                        return [
                            'id'   => $item->id,
                            'qty'  => $item->quantity,
                            'date' => $item->updated_at,
            ...
            // ✅ Group by product_id
            $groupedByProduct = $stockTracking->groupBy('product_id')->map(function ($rows) use ($default_store) {
                $productId = $rows->first()->product_id;

                // current stock by product
                $currentStockByProductQuery = DB::table('inv_current_stock')->where('product_id', $productId);
                if (!is_all_store()) {
                    $currentStockByProductQuery->where('store_id', $default_store);
                }
                $currentStockByProduct = $currentStockByProductQuery->sum('quantity');
                ...
                return [
                    ...
                    'in_total'      => $rows->where('movement', 'IN')->sum('quantity'),
                    'out_total'     => $rows->where('movement', 'OUT')->sum('quantity'),
                    'current_stock' => $currentStockByProduct,
                    'in_movements'  => $rows->where('movement', 'IN')->map(function ($item) {
                        return [
            ...
            return response()->json([
                'stocks'   => $groupedByStock,
                'products' => $groupedByProduct,
            ]);
        }
    }
