...
class StockTransferController_old extends Controller
{
    ...
    public function stockTransferHistory()
    {
        ...
        $products = CurrentStock::select(DB::raw('sum(quantity) as quantity'),
            ...
            ->groupby('product_id')
            ->get();
        $auto_gen = $this->transferNumberAutoGen();
        $all_transfers = StockTransfer::where('from_store','=',$store_id)
            ...
            ->select(DB::raw('
                ...
                SUM(accepted_qty) as received_quantity, 
                COUNT(*) as total_products, 
                MIN(status) as status
            '))
            ->orderBy('created_at', 'Desc')
            ->groupBy('transfer_no', 'from_store', 'to_store')->get();
