...
class GoodsReceivingController extends Controller
{
    ...
    //             $remaining_qty = $order_detail->quantity - $order_detail->received_quantity;
    ...
    //             $goods_receiving->expire_date = date('Y-m-d', strtotime($itemData['expiry_date']));
    ...
    //         $total_ordered = $order->details->sum('quantity');
    ...
    public function orderReceive(Request $request)
    {
        ...
        try {
            ...
            // 2. Process each item submitted from the modal.
            foreach ($validated['items'] as $itemData) {
                // Skip items where no quantity was received
    ...
    //     if ($order->status > 3) {
    //         return response()->json(['error' => 'Order already fully received'], 400);
    //     }
    ...
    //             throw new \Exception("Order not found or already fully received");
    //         }

    //         foreach ($items as $itemData) {
    //             $received_qty = (int)$itemData['quantity'];
                
    //             // Get order detail and validate quantities
    //             $order_detail = OrderDetail::find($itemData['purchase_order_detail_id']);
    //             if (!$order_detail) {
    //                 throw new \Exception("Purchase order detail not found for ID: " . $itemData['purchase_order_detail_id']);
    //             }

    //             // Check if we're not exceeding ordered quantity
    //             $remaining = $order_detail->quantity - $order_detail->received_quantity;
    //             if ($received_qty > $remaining) {
    //                 throw new \Exception("Cannot receive more than ordered quantity for product: " . $order_detail->product->name);
    //             }
    ...
    //             $goods_receiving = new GoodsReceiving();
    //             $goods_receiving->product_id = $itemData['product_id'];
    //             $goods_receiving->supplier_id = $supplier_id;
    //             $goods_receiving->quantity = $received_qty;
    //             $goods_receiving->unit_cost = $itemData['cost_price'];
    //             $goods_receiving->total_cost = $received_qty * $itemData['cost_price'];
    //             $goods_receiving->batch_number = $itemData['batch_number'];
    ...
    //             // Update or create stock with validation
    //             $stock = CurrentStock::firstOrNew([
    //                 'product_id' => $itemData['product_id'],
    //                 'batch_number' => $itemData['batch_number'],
    //                 'store_id' => $default_store_id
    ...
    //         } elseif ($total_received > 0) {
    //             $order->status = '2'; // Partial Received
    //         } else {
