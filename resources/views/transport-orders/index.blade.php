                    ...
                    <table id="fixed-header-transport-orders" class="display table nowrap table-striped table-hover" style="width:100%">
                        ...
                        @foreach($transportOrders as $order)
                            <tr>
                                ...
                                <td>{{ number_format($order->transport_rate, 2) }}</td>
                                <!-- <td>{{ $order->transport_rate }}</td> -->
                                <td>{{ number_format($order->advance_payment, 2) }}</td>
                                <td>{{ number_format($order->transport_rate - $order->advance_payment, 2) }}</td>
                                ...
                                <td style='white-space: nowrap'>
                                    <a href="#">
                                        <button class="btn btn-success btn-sm btn-rounded btn-show"
                                                ...
                                                data-payment_status="{{$order->payments->sum('amount') >= $order->transport_rate ? 'Completed' : 'Not Completed'}}"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div id="show_payment_status" class="pt-2"></div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>Notes</label>
                                <textarea class="form-control" id="show_notes" rows="1" disabled></textarea>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal fade" id="editOrder" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content shadow-sm border-0">
                ...
                <div class="col-md-4 pickup-location-group">
                    <div class="form-group">
                        <label>Pickup Location <span class="text-danger">*</span></label>
                        <!-- Factory (Suppliers) -->
                        <select name="pickup_location" class="form-control pickup-supplier" id="pickup_supplier_edit">
                        ...
                        <!-- Inter-Branch (Stores) -->
                        <select name="pickup_location" class="form-control pickup-store d-none" id="pickup_store_edit" disabled>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select name="delivery_location" class="form-control" id="delivery_location_edit" required>
                            <option value="">Select Store</option>
                            @foreach($stores as $store)
                                <option value="{{ $store->name }}">{{ $store->name }}</option>
                            @endforeach
    ...
    <script>
        $(document).ready(function () {
            $('#fixed-header-transport-orders').DataTable({
                order: [[7, 'desc']], // Sorts by the hidden created_at column (now at position 8)
            });
        ...
        document.querySelectorAll('.btn-show').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if(paymentStatus == 'Completed') {
                    paymentStatusBadge = '<span class="badge badge-success">Completed</span>';
                } else {
                    paymentStatusBadge = '<span class="badge badge-warning">Not Completed</span>';
                }
                document.getElementById('show_payment_status').innerHTML = paymentStatusBadge;
...
document.querySelectorAll('.btn-edit').forEach(button => {
    button.addEventListener('click', function() {
        ...
        // Set order type and pickup location
        const orderType = this.getAttribute('data-order_type');
        const pickupLocation = this.getAttribute('data-pickup_location');
        
        // Initialize the correct pickup field based on order type
        if (orderType === 'factory') {
            document.querySelector('.pickup-supplier').classList.remove('d-none');
            document.querySelector('.pickup-supplier').disabled = false;
            document.querySelector('.pickup-store').classList.add('d-none');
            document.querySelector('.pickup-store').disabled = true;
            document.getElementById('pickup_supplier_edit').value = pickupLocation;
        } else {
            document.querySelector('.pickup-store').classList.remove('d-none');
            document.querySelector('.pickup-store').disabled = false;
            document.querySelector('.pickup-supplier').classList.add('d-none');
            document.querySelector('.pickup-supplier').disabled = true;
            document.getElementById('pickup_store_edit').value = pickupLocation;
        }
...
// Handle order type change in edit modal
document.addEventListener('DOMContentLoaded', function() {
    const orderTypeEdit = document.getElementById('order_type_edit');
    if (orderTypeEdit) {
        orderTypeEdit.addEventListener('change', function() {
            const supplier = document.querySelector('.pickup-supplier');
            const store = document.querySelector('.pickup-store');
